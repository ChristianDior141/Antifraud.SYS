...
export default function LoginPage() {
  ...
  const onSubmit = async (data: FormData) => {
    setError('');
    const result = await dispatch(login({ ...data, mfaCode: mfaCode || undefined }));
    if (login.fulfilled.match(result)) {
      navigate('/dashboard');
    } else {
      const msg = result.payload as string;
      if (msg && msg.toLowerCase().includes('mfa')) {
        setMfaRequired(true);
        setError(mfaCode ? msg : 'Введите код двухфакторной аутентификации');
      } else {
        setError(msg);
      }
    }
        ...
        <div className="w-full max-w-md">
          <div className="rounded-2xl bg-white p-8 shadow-sm ring-1 ring-gray-200">
            ...
            <form onSubmit={handleSubmit(onSubmit)} className="mt-6 space-y-4">
              ...
              {mfaRequired && (
                <div>
                  <label className="block text-sm font-medium text-gray-700">Код 2FA</label>
                  <input
                    value={mfaCode}
